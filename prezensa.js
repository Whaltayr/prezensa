...
const PRODUCTS = [
  {
    ...
    desc_pt:
      "Um opaco e misterioso véu de madeiras raras, projetado para o homem que deixa marcas indeléveis — nas salas e nas memórias.",
    desc_en:
      "An opaque and mysterious veil of rare woods, designed for the man who leaves indelible marks — in rooms and in memories.",
  ...
  },
  {
    ...
    desc_pt:
      "A tempestade que antecede a calmaria. Cedro e incenso em camadas que revelam complexidade sem pressa.",
    desc_en:
      "The storm before the calm. Cedar and incense in layers that reveal complexity without hurry.",
  ...
  },
  {
    ...
    desc_pt:
      "O horizonte infinito captado em vidro. Fresco, elegante e surpreendentemente profundo.",
    desc_en:
      "The infinite horizon captured in glass. Fresh, elegant and surprisingly deep.",
  ...
  },
  {
    ...
    desc_pt:
      "As rotas antigas do oriente em uma única fragrância. Opulento, raro, intransigente.",
    desc_en:
      "The ancient routes of the Orient in a single fragrance. Opulent, rare, uncompromising.",
    top: ["Açafrão", "Rosa"],
    heart: ["Oud", "Cânfora"],
  ...
  },
  {
    ...
    desc_en:
      "An ethereal dance of silk and morning light. Femininity that needs no announcement.",
  ...
  },
  {
    ...
    desc_pt:
      "A rosa que floresce depois do por do sol. Intensa, sedutora e completamente inesquecível.",
    desc_en:
      "The rose that blooms after sunset. Intense, seductive and completely unforgettable.",
  ...
  },
  {
    ...
    desc_pt:
      "Um jardim de verão em movimento. Frescor que persiste delicadamente ao longo do dia.",
    desc_en:
      "A summer garden in motion. Freshness that lingers delicately throughout the day.",
    top: ["Pera", "Néroli"],
  ...
  },
  {
    ...
    desc_pt:
      "Veludo invisível sobre a pele. Uma fragrância que envolve como um abraço quente e inesperado.",
    desc_en:
      "Invisible velvet on the skin. A fragrance that wraps like a warm, unexpected embrace.",
...
splash.querySelectorAll(".splash__lang-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    splash
      .querySelectorAll(".splash__lang-btn")
...
function initScrollAnimations() {
  ...
  // ── ARTE ──────────────────────────────────────────
  document
    .querySelectorAll(".arte__content > *:not(.arte__pillars)")
    .forEach((el, i) => {
      scrollReveal(
        el,
        { opacity: 0, y: 20 },
  ...
  document
    .querySelectorAll(".contato__form .form-field, .contato__form .btn-lux")
    .forEach((el, i) => {
      scrollReveal(
        el,
        { opacity: 0, y: 18 },
...
function initProductCards() {
  ...
  setTimeout(() => {
    const visible = Array.from(
      document.querySelectorAll(".prod-card:not(.hidden)"),
    );
...
function applyFilters() {
  ...
  allCards.forEach((card) => {
    ...
    const famMatch =
      familia === "all" || (card.dataset.familia || "").includes(familia);
    const ocaMatch =
      ocasiao === "all" || (card.dataset.ocasiao || "").includes(ocasiao);
    const intMatch =
      intensidade === "all" ||
      (card.dataset.intensidade || "").includes(intensidade);
...
document.getElementById("featuredAdd")?.addEventListener("click", () => {
  ...
  const name =
    currentMode === "gent" ? btn.dataset.nameGent : btn.dataset.nameLady;
  const price =
    currentMode === "gent" ? btn.dataset.priceGent : btn.dataset.priceLady;
...
function openPyramid(index) {
  ...
  document.getElementById("pyramidDesc").textContent =
    currentLang === "pt" ? p.desc_pt : p.desc_en;
  document.getElementById("pyramidPrice").textContent =
    `R$ ${p.price.toLocaleString("pt-BR")}`;

  ["pyramidTop", "pyramidHeart", "pyramidBase"].forEach((id, i) => {
    document.getElementById(id).innerHTML = [p.top, p.heart, p.base][i]
      .map((n) => `<span>${n}</span>`)
      .join("");
  });
...
document.querySelectorAll(".prod-card__detail-btn").forEach((btn) => {
  btn.addEventListener("click", () =>
    openPyramid(parseInt(btn.dataset.product)),
  );
});
...
document.getElementById("contatoForm")?.addEventListener("submit", (e) => {
  ...
  btn.innerHTML =
    currentLang === "pt" ? "✓ Mensagem enviada!" : "✓ Message sent!";
  setTimeout(() => {
    btn.innerHTML = orig;
  }, 3000);
});
